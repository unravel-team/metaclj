...
1. Installing tools (generally you only need to do this once)
   - =install-antq= (See: <antq section>)
   - =install-kondo-configs= (See: <clj-kondo section>)
   - =install-zprint-config= (See: <zprint section>)
   - =install-gitignore= (See: <gitignore section>)
2. Running our Clojure project
   - =repl= (See: <REPL section>)
   - =repl-enrich= (See: <Enrich section>)
3. Testing our Clojure project
   - =check= (See: <Checking tags, linting and formatting section>)
   - =test= (See: <Polylith Tests section>)
   - =test-coverage= (See: <Cloverage section>)
4. Upgrading libraries and tooling
   - =upgrade-libs= (See: <antq section>)
